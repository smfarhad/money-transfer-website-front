                    ...
                    <table
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><a href="{{ url('customer-profile', [$row->customer_id]) }}">{{$row->customer_name}}</a></td>
                                <td><a href="{{ url('customer-profile', [$row->customer_id]) }}">{{$row->personnumber}}</a></td>
                                <td>{{$row->recipient_name}}</td>
                                 <td>{{-- $row->depositaedAmount --}}{{$row->source_amount+$row->fees*0}}</td>
                                <td><a href="{{ url('transection-completed', [$row->user_id]) }}">{{$row->user_name}}</a></td>
                                <td>{{date('Y-m-d', strtotime($row->create))}}</td>
                                <td>{{date('Y-m-d', strtotime($row->date_execution))}}</td>
